...
DWORD WINAPI PostJson(LPVOID lpParameter)
{
    ...
    HINTERNET hInternet = InternetOpen("WinInet JSON POST", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
    if (!hInternet) {
    ...
    HINTERNET hConnect = InternetConnect(hInternet, data->host.c_str(), INTERNET_DEFAULT_HTTPS_PORT,
    ...
    HINTERNET hRequest = HttpOpenRequest(hConnect, "POST", data->path.c_str(), NULL, NULL,
    ...
    if (!HttpSendRequest(hRequest, headers.c_str(), headers.length(), (LPVOID)jsonString.c_str(),
                         jsonString.length())) {
