<template>
    <div style="height: 100%; display: flex; justify-content: center; align-items: center">
        ...
        <div v-else class="text-grey text-center">
            ...
            <p class="font-weight-light text-grey-darken-1 mt-2">Add a window from the top menu above</p>
            <p v-if="auth.user" class="font-weight-light text-grey-darken-1 mt-2">
                {{ auth.user.personal.name_full }}, {{ auth.user.cid }},
                {{ auth.user.vatsim.rating.short }} from {{ auth.user.vatsim.subdivision.name }}
            </p>
