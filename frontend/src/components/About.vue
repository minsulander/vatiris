<template>
    <div class="text-grey-lighten-1 text-body-2">
        ...
        <p class="mt-3" v-if="auth.user">
            ...
            <span v-if="auth.user.vatsim.subdivision && auth.user.vatsim.subdivision.name"> from {{ auth.user.vatsim.subdivision.name }}</span>
            <span v-else-if="auth.user.vatsim.division && auth.user.vatsim.division.name"> from {{ auth.user.vatsim.division.name }}</span>
        </p>
        <p class="mt-3">
