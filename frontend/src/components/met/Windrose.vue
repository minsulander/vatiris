        ...
        <div class="runway-selector">
          <template v-for="rwy in availableRunways" :key="rwy">
            <a
              ...
              class="pa-1 font-weight-bold text-caption"
              @click="currentRunway = rwy"
...
const windData = computed(() => {
  ...
  return {
    ...
    speed: data.speed || 0,
    vrbFrom: data.variableFrom,
    vrbTo: data.variableTo,
    minWind: data.minWind || 0,
    maxWind: data.maxWind || 0,
    headWind: runwayData?.headWind || 0,
    crossWind: runwayData?.crossWind || 0,
    crossWindDir: runwayData?.crossWindDir || 'L'
  };
});
...
const tenDegreePoints = Array.from({ length: 36 }, (_, i) => i * 10)
...
onMounted(() => {
  ...
  if (props.id) {
    ...
    if (!currentRunway.value && availableRunways.value.length > 0) {
      currentRunway.value = availableRunways.value[0];
    }

    // Add outdated check interval
    checkOutdatedInterval = setInterval(() => {
      outdated.value = lastUpdate.value.length > 0 && 
...
watch(() => windStore.windData[props.id], (newValue, oldValue) => {
  if (!newValue) return
...
const getVrbArcPath = computed(() => {
  ...
  const sweepAngle = needsCrossover 
